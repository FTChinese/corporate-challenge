...
(function () {
    ...
    var Swiper = function Swiper(container, params) {
        ...
        for (var def in defaults$$1) {
            if (typeof params[def] === 'undefined') {
                params[def] = defaults$$1[def];
            } else if (_typeof(params[def]) === 'object') {
                for (var deepDef in defaults$$1[def]) {
                    if (typeof params[def][deepDef] === 'undefined') {
                        params[def][deepDef] = defaults$$1[def][deepDef];
                    }
                }
            }
        }
...
var Share$1 = function () {
	...
	createClass(Share, [{
		...
		value: function generateSocialUrl(socialNetwork) {
...
var ContentLoader = function () {
	function ContentLoader(rootEl, config) {
	...
	createClass(ContentLoader, [{
		key: 'loadContent',
		value: function loadContent(url, className) {
			var _this = this;

			ajax.getData(url, function (err, data) {
				if (err) return;

				var articleEl = convertMd(data, className);

				_this.rootEl.appendChild(articleEl);
...
var Toggle = function () {
	function Toggle(toggleEl, config) {
		...
		var parentNode = toggleEl.parentNode;
...
var Expander = function () {
	function Expander(rootEl, config) {
		var _this = this;

		classCallCheck(this, Expander);
		...
		this.handleClick();
		this.rootEl.addEventListener('click', function (e) {
			if (e.target === _this.toggleEl) {
				_this.handleClick();
			}
		});
	}

	createClass(Expander, [{
		key: 'handleClick',
		...
		value: function init(el, opts) {
			...
			var expanderEls = el.querySelectorAll('[data-o-component=o-expander]');
			if (!expanderEls) {
				return;
			}
			for (var i = 0; i < expanderEls.length; i++) {
